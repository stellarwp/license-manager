...
/**
 * Option-backed repository for the product portal.
 *
 ...
 * @since 1.0.0
 */
class Portal_Repository {

	use With_Debugging;
	use With_Error_Throttle;

	/**
	 * Option name for the portal state envelope.
	 *
	 * Stores an associative array with four keys:
	 *   - collection      (array|null)     Portal_Collection::to_array() from the last
	 ...
	 * @var string
	 */
	public const PORTAL_STATE_OPTION_NAME = 'lw_harbor_portal_state';

	/**
	 * State envelope key for the serialized portal collection array.
	 *
	 * @since 1.0.0
	 *
	 * @var string
	...
	/**
	 * In-memory cache of the deserialized portal collection for the current request.
	 *
	 * @since 1.0.0
	 *
	 * @var Portal_Collection|null
	 */
	 ...
	 * @since 1.0.0
	 *
	 * @param Portal_Client $client The portal client to fetch from.
	 */
	...
	/**
	 * Gets the full portal, using the stored option when available.
	 *
	...
	/**
	 * Returns the cached portal collection without making any API call.
	 *
	 ...
	 * @since 1.0.0
	 *
	 * @return Portal_Collection|null
	 */
	public function get_cached(): ?Portal_Collection {
		if ( $this->cached_collection !== null ) {
			return $this->cached_collection;
		}

		$state = $this->read_portal_state();

		if ( is_array( $state[ self::STATE_KEY_COLLECTION ] ) ) {
			$this->cached_collection = Portal_Collection::from_array( $state[ self::STATE_KEY_COLLECTION ] );
	...
	protected function fetch() {
		...
		$this->set_portal( $result );

		return $result;
	}

	/**
	 * Persist the portal collection or a fetch error to the state option.
	 *
	 * On success (Portal_Collection): updates collection and last_success_at,
	 * clears last_error.
	 *
	 ...
	 * @since 1.0.0
	 *
	 * @param Portal_Collection|WP_Error $data The portal or fetch error to store.
	 *
	 * @return void
	 */
	public function set_portal( $data ): void {
		if ( $data instanceof Portal_Collection ) {
			...
			update_option( self::PORTAL_STATE_OPTION_NAME, $state, false );
	...
	/**
	 * Delete the entire portal state option.
	 *
	 * @since 1.0.0
	 *
	 * @return void
	 */
	public function delete_portal(): void {
		$this->cached_collection = null;
		delete_option( self::PORTAL_STATE_OPTION_NAME );
	}

	/**
	 * Unix timestamp of the last successful portal fetch, or null if never fetched.
	 *
	 * @since 1.0.0
	 *
	 * @return int|null
	...
	public function get_last_success_at(): ?int {
		$value = $this->read_portal_state()[ self::STATE_KEY_LAST_SUCCESS_AT ];

		return is_int( $value ) ? $value : null;
	}

	/**
	 * Unix timestamp of the most recent failed portal fetch, or null if no
	...
	public function get_last_failure_at(): ?int {
		$value = $this->read_portal_state()[ self::STATE_KEY_LAST_FAILURE_AT ];
	...
	public function get_last_error(): ?WP_Error {
		$error = $this->read_portal_state()[ self::STATE_KEY_LAST_ERROR ];

		return $error instanceof WP_Error ? $error : null;
	}

	/**
	 * Read the raw portal state array from the option, returning a zeroed
	 ...
	 */
	private function read_portal_state(): array {
		$raw = get_option( self::PORTAL_STATE_OPTION_NAME, null );
