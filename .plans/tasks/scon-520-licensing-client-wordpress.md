...
Keep `Product_Entry` as Harbor's own DTO but update it to hydrate from the package's `PortalEntry` type instead of from raw API response arrays.

Migrate the Portal `Http_Client` from its PSR-18/Symfony stack to `WordPressHttpClient` as well. Once both subsystems are on the WordPress transport, drop `symfony/http-client` from `composer.json`.
