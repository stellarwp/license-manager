...
final class Portal_RepositoryTest extends HarborTestCase {
	...
	protected function setUp(): void {
		...
		$client           = new Fixture_Client( codecept_data_dir( 'portal/default.json' ) );
	...
	public function test_get_stores_in_option(): void {
		...
		$state = get_option( Portal_Repository::PORTAL_STATE_OPTION_NAME );
	...
	public function test_get_serves_from_option(): void {
		...
		update_option( Portal_Repository::PORTAL_STATE_OPTION_NAME, $stale );
	...
	public function test_get_preserves_collection_on_error(): void {
		...
		$error      = new WP_Error( 'portal_error', 'API unavailable.' );
		$bad_client = $this->makeEmpty( Portal_Client::class, [ 'get_portal' => $error ] );
		( new Portal_Repository( $bad_client ) )->refresh();
	...
	public function test_get_cached_returns_null_when_only_error_stored(): void {
		$this->repository->set_portal( new WP_Error( Error_Code::INVALID_RESPONSE, 'API failure' ) );
	...
	public function test_get_cached_does_not_trigger_api_fetch(): void {
		$client = $this->makeEmpty(
			...
			[
				'get_portal' => function () {
					$this->fail( 'get_portal() should not be called from get_cached().' );
				},
			]
		);
	...
	public function test_set_portal_invalidates_in_memory_cache(): void {
		$this->repository->get();

		$before = $this->repository->get_cached();

		// Overwrite with a different collection.
		$this->repository->set_portal(
	...
	public function test_delete_portal_invalidates_in_memory_cache(): void {
		$this->repository->get();
	...
	public function test_set_portal_error_invalidates_in_memory_cache(): void {
		$this->repository->get();

		$this->assertNotNull( $this->repository->get_cached() );

		$this->repository->set_portal( new WP_Error( Error_Code::INVALID_RESPONSE, 'fail' ) );
	...
	public function test_set_portal_collection_clears_last_failure_at(): void {
		$this->repository->set_portal( new WP_Error( Error_Code::INVALID_RESPONSE, 'API failure' ) );
